...
class T411():
    ...
    def __init__(self):
        ...
        self.api = t411api.T411API()

        self.login_done = False
        
    def searchterm(self, artist, album, year, format):
    ...
    def _doLogin(self, login, password):
        ...
        logger.debug(u"T411 logged")
        self.login_done = True
        #self.opener.open(self.url + '/users/login', data)
    
    def search(self, searchterm, maxsize, minseeders, albumid, bitrate):
        
        if not self.login_done:
            self._doLogin( headphones.CONFIG.TONZE_LOGIN, headphones.CONFIG.TONZE_PASSWORD )

        r = self.api.search(searchterm,cat=395,subcat=623)

        #results = []
        logger.debug(u"Search result: " + json.dumps(r))
        return r['torrents']
    ...
    def get_torrent(self, url, torrent_name, savelocation=None):
        ...
        logger.info('[T411] getting torrent %s: %s',torrent_name,url)
        torrent_name = torrent_name
        
        if savelocation:
            dir = savelocation
        else:
            dir = mkdtemp(suffix='_t411_torrents')
        try:
            download_path = os.path.join(dir, torrent_name)
            logger.info('[T411] try saving torrent (%s) to: %s',url,download_path)
            self.api.download(int(url),torrent_name,dir)
            ...
            logger.error('Error getting torrent: %s' % str(e))  
            return False     
         
        logger.info('[T411] torrent saved: %s',download_path)
