    ...
    if kind == 'nzb':
        ...
        if headphones.CONFIG.TORRENT_DOWNLOADER == 0:
            ...
            if bestqual[2].lower().startswith("magnet:"):
                ...
                if bestqual[3] == 'T411':
                    torrent_name = helpers.replace_illegal_chars(folder_name) + '.torrent'
                    file_or_url, _hash = t411.get_torrent(bestqual[2],torrent_name,headphones.CONFIG.TORRENTBLACKHOLE_DIR)
                    return
                elif not torrent_to_file(download_path, data):
                    return
            ...
            if bestqual[3] == 'rutracker.org':
                ...
                logger.info('[T411] torrent name: %s',torrent_name)
            ...
            if bestqual[3] == 'T411':
                try:
                   shutil.rmtree(os.path.split(file_or_url)[0])
                    ...
                    logger.exception("Exception when removing tmp t411 torrent file")
            ...
            try:
                ...
                if bestqual[3] == 'rutracker.org':
                    ...
                    torrent_name = helpers.replace_illegal_chars(folder_name) + '.torrent'
                    file_or_url, _hash = t411.get_torrent(bestqual[2],torrent_name)
                    logger.info('[T411] file or url: %s',file_or_url)
                    torrentid = deluge.addTorrent(file_or_url)
                    # remove temp .torrent file created above
                    #try:
                    #   shutil.rmtree(os.path.split(file_or_url)[0])
                    #except Exception as e:
...
def searchTorrent(album, new=False, losslessOnly=False, albumlength=None,
    ...
    if headphones.CONFIG.TONZE:
        ...
        if not usersearchterm:
            searchterm = t411.searchterm(artistterm, albumterm, year, format)
        else:
            searchterm = t411.searchterm(usersearchterm, ' ', ' ', format)

        logger.info(u'Parsing results from %s' % searchterm)
            
            # parse results and get best match
            
        t11list = t411.search(searchterm, maxsize, minimumseeders, albumid, bitrate)
            
        # add best match to overall results list
            
        if t11list:
            for t11torr in t11list:
                try:
                    title = t11torr['name'].decode('utf-8')
                except:
                    title=t11torr['name']
                size = t11torr['size']
                #url = t11torr['url']
                id = t11torr['id']
                resultlist.append((title, size, id, provider, 'torrent', True))
                logger.info('Found %s. Size: %s' % (title, helpers.bytes_to_mb(size)))
        else:
